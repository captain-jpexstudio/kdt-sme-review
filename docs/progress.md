...
## 현재 위치
**▶ P0 진행** — repo 골조 생성 완료(backend/frontend/docker/alembic/seed). Python 전체 컴파일·active_edit 단위검증 통과.
**남은 DoD**: 실기기에서 `docker compose up` → `alembic upgrade head` 통과 + `/api/v1/health` 200 확인(아직 미실행).

## 마일스톤 (spec-v6 §18)
| Phase | 산출물 | DoD | 상태 |
|---|---|---|---|
| **P0** 셋업 | repo·docker·DB·Alembic·시드·파일스토리지 | `compose up` 기동·마이그레이션 통과 | 🟡 골조완료·compose 미검증 |
